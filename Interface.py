...
class mainProgramme(Tk):
	...
	def setGraph(self):
		self.spAmpl.cla()
		self.spAmpl.clear()
		#self.spAmpl.set_xscale('log')
		self.spAmpl.grid(b=True, which='both', axis='both')
		self.spAmpl.set_xbound(self.freq_min, self.freq_max)
		
		self.spPhas.cla()
		self.spPhas.clear()
		#self.spPhas.set_xscale('log')
		self.spPhas.grid(b=True, which='both', axis='both')
		self.spPhas.set_xbound(self.freq_min, self.freq_max)
		...
		if(self.scan==1):
			...
			if newStr=="END\r\n":
				...
				if(len(values)==3):
					...
					freq=(int(values[2], 16)*546875)>>21
	...
	def __init__(self):
		...
		self.spAmpl.set_title('Amplitude', fontdict={'fontsize':10})
		self.spAmpl.autoscale(enable=False, axis='x')
		self.spAmpl.grid(b=True, which='both', axis='both')
		
		
		self.spPhas = self.fgBode.add_subplot(212)
		self.spPhas.set_title('Phase', fontdict={'fontsize':10})
		self.spPhas.autoscale(enable=False, axis='x')
		self.spPhas.grid(b=True, which='both', axis='both')
		
		frPlt = Frame(self)
		frPlt.grid(row=0, column=1,rowspan=2,padx=5,pady=5)
		...
		labePer = Label(frUser, text="Peripherique")
		labePer.pack()
		...
		labeDo.pack()
		self.spinDo = Spinbox(frUser, from_=50, to=19999999, value=4913700)
		self.spinDo.pack()
		
		labeUp = Label(frUser, text="Stop frequency (Hz)")
		labeUp.pack()
		self.spinUp = Spinbox(frUser, from_=51, to=20000000, value=4914100)
		self.spinUp.pack()
		
		labePas = Label(frUser, text="Frequency step (Hz)")
		labePas.pack()
		self.spinPas = Spinbox(frUser, from_=1, to=10000000)
		self.spinPas.pack()
		...
		labePps.pack()
		self.spinPps = Spinbox(frUser, from_=1, to=1000)
		self.spinPps.pack()
